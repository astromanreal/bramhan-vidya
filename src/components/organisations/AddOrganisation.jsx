...
export default function AddOrganisation() {
  ...
  const handleSubmit = async (event) => {
    ...
    const filteredOrg = {
      ...organisation,
      randomKeyDetails: organisation.randomKeyDetails.filter(
        (detail) => detail.key && detail.value
      ),
      notes: organisation.notes.filter((note) => note.key || note.value),
    };
    try {
      const response = await axios.post(
        `${apiUrl}/organisations/addorganisation`,
        filteredOrg
      );
  ...
  const addRandomKeyDetail = () => {
    setOrganisation((prevOrg) => ({
      ...prevOrg,
      randomKeyDetails: [...prevOrg.randomKeyDetails, { key: "", value: "" }],
    }));
  };
  ...
  return (
    <>
      <form onSubmit={handleSubmit} id="add-profile-form">
        ...
        <button type="submit">Submit Organisations</button>
      </form>
    </>
  );
}
